...
def prepare_data(size, len=-1, color=True, input_list=None, path=None, use_tqdm=False):
...
def sketche_scale(x):
    return 1 - np.random.uniform(0.7, 1) * (1 - x)


def Hint_binarymask(img):
    img = misc.imresize(img, [img.shape[0] // 2, img.shape[1] // 2])
    mask = np.random.uniform(0, 1, img.shape[0:2]) > np.abs(np.random.normal(loc=1, scale=0.005, size=img.shape[0:2]))
    img = preprocessing(img * np.expand_dims(mask, -1))
    return img
